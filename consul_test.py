...
PAYLOAD_CATALOG_DEREGISTER = {
    ...
    "port": PAYLOAD_CATALOG_REGISTER["port"],
...
def test_catalog_register():
    ...
    assert consul.catalog.register.register(call=serviceCall) == True
...
def test_catalog_deregister():
    ...
    assert consul.catalog.deregister.deregister(call=serviceCall) == True
...
def test_kv_create_or_update():
    ...
    assert consul.kv.create_or_update.create_or_update(call=serviceCall) == True

def test_kv_delete():
    serviceCall = ServiceCall(
        domain=DOMAIN, service="kv_delete", data=PAYLOAD_KV_DELETE, context=Context())

    assert consul.kv.delete.delete(call=serviceCall) == True